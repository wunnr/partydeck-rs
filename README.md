...
Download the latest release [here](https://github.com/wunnr/partydeck-rs/releases). Extract the zip into a folder.
...
**IMPORTANT:** Steam Input interferes with PartyDeck's gamepad detection, causing issues like duplicate controllers showing up! If PartyDeck is added as a non-Steam game, go to the properties in Steam and make sure that Steam Input is disabled. If Steam is open at all, make sure that your gamepads aren't using a Desktop layout.

Any gamepad that SDL supports is theoretically supported: Specifically, Xbox/PlayStation/Switch gamepads have all been tested with PartyDeck. Some native Linux games use older versions of SDL that have incorrect mappings with newer gamepads, so the launcher has an option to force a game to use the Steam Runtime's version of SDL, which usually fixes this.
...
- **Gamescope:** Contains each instance of the game to its own window. Also has the neat side effect of receiving gamepad input even when the window is not currently active, meaning multiple Gamescope instances can all receive input simultaneously
- **Bubblewrap:** Uses bindings to mask out evdev input files from the instances, so each instance only receives input from one specific gamepad. Also uses directory binding to give each player their own save data and settings within the games.
...
- "Console-like splitscreen experience" means single-screen and gamepads only. Multi-monitor support is possible but will require a better understanding of the KWin Scripting API. Support for multiple keyboards and mice is also theoretically possible, but I'll have to look into how I would go about implementing it.
...
- Gamepad navigation support in the launcher is super primitive; I'd love to try making a more gamepad-friendly, Big-Picture-style UI in the future, but have no immediate plans for it.
